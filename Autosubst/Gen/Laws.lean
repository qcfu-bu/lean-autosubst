/-
# σ-calculus law generation (the `asimp` rewrite set).

Emits the `asimp`-facing rewrite set **stated over the typeclass-method / notation forms**
...
/-- All notation-native `asimp` lemmas for `sig`: the per-constructor push laws and the method-form
fusion / identity / variable laws. (Per the Coq reference, `up` is *unfolded* by `asimp` rather than
kept folded, so there are no extra up-fusion laws — the static `scons`/`funcomp` laws close the
calculus after the generated `up_b_v` equation lemmas fire.) Emitted after the notation instances so
the `Subst{k}`/`Var` methods resolve. -/
