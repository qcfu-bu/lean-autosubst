...
def genNotationCommands (sc : Bool) (sig : Signature) : CommandElabM (Array (TSyntax `command)) := do
  ...
  for comp in sig.components do
    for si in substSortsOf sig comp do
      ...
      | [_], [sm], [rm] =>
        out := out.push (← `(command| instance : $subst1I $dom $sm $cod := ⟨$substId⟩))
        out := out.push (← `(command| instance : $ren1I $dom $rm $cod := ⟨$renId⟩))
      ...
      | [_, _], [sm1, sm2], [rm1, rm2] =>
        out := out.push (← `(command| instance : $subst2I $dom $sm1 $sm2 $cod := ⟨$substId⟩))
        out := out.push (← `(command| instance : $ren2I $dom $rm1 $rm2 $cod := ⟨$renId⟩))
        out := out.push (← `(command|
      ...
      if si.isOpen then
        ...
        out := out.push (← `(command| instance : $varCls $dom $idxTy := ⟨$varCtor⟩))
        out := out.push (← `(command|
  ...
  | [s] =>
    ...
    let cod ← if sc then `(Fin ($(scopeVar "m" s) + 1) → $(mkIdent s) ($(scopeVar "n" s) + 1))
              else `(Nat → $(mkIdent s))
    out := out.push (← `(command| instance : $upI $dom $cod := ⟨$upId⟩))
    out := out.push (← `(command|
